(function () {
  ...
  function initOriginCircle() {
    const root = document.querySelector("[data-origin-circle]");
    if (!root) return;
    const chapters = (CKM.coffeeOrigin && CKM.coffeeOrigin.chapters) || [];
    if (!chapters.length) return;
    const esc = CKMSections.esc;
    const panel = root.querySelector("[data-origin-panel]");
    const live = root.querySelector("[data-origin-live]");
    const progress = root.querySelector("[data-origin-progress]");
    const prevBtn = root.querySelector("[data-origin-prev]");
    const nextBtn = root.querySelector("[data-origin-next]");
    const end = root.querySelector("[data-origin-end]");
    const webglHost = root.querySelector("[data-origin-webgl]");
    const fallback = root.querySelector("[data-origin-fallback]");
    const reduced =
    ...
    function hashIndex() {
      ...
      const found = chapters.findIndex((ch, i) => (ch.id || `origin-${String(i + 1).padStart(2, "0")}`) === id);
      return found >= 0 ? found : 0;
    }

    function paintPanel(i, announce) {
      const ch = chapters[i];
      if (!ch || !panel) return;
      panel.classList.add("is-swap");
      window.setTimeout(() => {
        root.querySelector("[data-origin-kicker]").textContent = ch.label || ch.kicker || "";
        root.querySelector("[data-origin-title]").textContent = ch.title || "";
        root.querySelector("[data-origin-copy]").innerHTML = String(ch.text || "")
          .split(/\n\n/)
          .filter(Boolean)
          .map((p) => `<p>${esc(p)}</p>`)
          .join("");
        root.querySelector("[data-origin-caption]").textContent = ch.caption || "";
        panel.classList.remove("is-swap");
      }, 90);
      if (progress) progress.textContent = `${String(i + 1).padStart(2, "0")} / ${String(chapters.length).padStart(2, "0")}`;
      if (prevBtn) prevBtn.disabled = i === 0;
      if (nextBtn) nextBtn.disabled = i === chapters.length - 1;
      if (end) end.hidden = i !== chapters.length - 1;
      root.querySelectorAll("[data-origin-goto]").forEach((el) => {
        const on = Number(el.getAttribute("data-origin-goto")) === i;
        if (el.tagName === "A") el.setAttribute("aria-current", on ? "page" : "false");
        else el.setAttribute("aria-current", on ? "true" : "false");
      });
      if (announce && live) live.textContent = `${ch.label || ""} ${ch.title || ""}`.trim();
    }

    function select(i, source) {
      const next = Math.max(0, Math.min(chapters.length - 1, i));
      if (next === index && source !== "hash") {
        paintPanel(next, false);
        return;
      }
      index = next;
      window.__originChapter = index;
      paintPanel(index, source !== "init");
      const id = chapters[index].id || `origin-${String(index + 1).padStart(2, "0")}`;
      if (source !== "hash" && history.replaceState) {
        history.replaceState(null, "", `#${id}`);
      }
      if (gallery && source !== "gallery") {
        applying = true;
        gallery.goTo(index, source === "init");
        window.setTimeout(() => {
          applying = false;
        }, 80);
      }
    ...
    index = window.__originChapter != null ? window.__originChapter : hashIndex();
    paintPanel(index, false);
    ...
    prevBtn?.addEventListener("click", () => select(index - 1, "controls"));
    nextBtn?.addEventListener("click", () => select(index + 1, "controls"));
    root.querySelector("[data-origin-restart]")?.addEventListener("click", () => select(0, "controls"));
    root.querySelectorAll("[data-origin-goto]").forEach((el) => {
      el.addEventListener("click", (event) => {
        if (el.tagName === "A") event.preventDefault();
        select(Number(el.getAttribute("data-origin-goto")), "link");
      });
    });
    root.querySelector("[data-origin-read-all]")?.addEventListener("click", () => {
      const full = root.querySelector("[data-origin-full]");
      const btn = root.querySelector("[data-origin-read-all]");
      const open = full.hasAttribute("hidden");
      if (open) full.removeAttribute("hidden");
      else full.setAttribute("hidden", "");
      btn.setAttribute("aria-expanded", open ? "true" : "false");
      btn.textContent = open ? "Hide full story" : "Read all chapters";
    });

    const onHash = () => select(hashIndex(), "hash");
    window.addEventListener("hashchange", onHash);
    window._originHashHandler = onHash;

    const startGallery = (mod) => {
      if (!webglHost || !mod.hasWebGL() || reduced) {
        enableFallback();
        select(index, "init");
        return;
      }
      enableWebGL();
      if (root._gallery && root._gallery.destroy) root._gallery.destroy();
      applying = true;
      gallery = mod.mount(webglHost, {
        items: chapters.map((ch, i) => ({
          image: ch.image,
          text: ch.label || `${String(i + 1).padStart(2, "0")} · ${ch.kicker}`,
          chapterIndex: i,
          focus: ch.focus || [0.5, 0.5],
        })),
        bend: 1.5,
        textColor: "#24483D",
        borderRadius: 0.025,
        ...
        onSettled(chapter, meta) {
          if (applying) return;
          if (meta && meta.programmatic) return;
          select(chapter, "gallery");
        },
      });
      root._gallery = gallery;
      select(index, "init");
  ...
  function paint() {
    const existingGallery = document.querySelector("[data-origin-circle]");
    if (existingGallery && existingGallery._gallery && existingGallery._gallery.destroy) {
      existingGallery._gallery.destroy();
    }
    if (window._originHashHandler) {
      window.removeEventListener("hashchange", window._originHashHandler);
      window._originHashHandler = null;
    }
    const main = document.getElementById("main");
